...
const iif = (test, fn, ...args) => (test ? fn.apply(null, args) : gutil.noop());
...
class JsBuild {
    ...
    copy() {
        gulp.src(this.conf.path.src)
            .pipe(gulp.dest(this.conf.path.dest));
    }
    copylibrary() {
        del(this.conf.path.dest)
            .then(() => gulp.src(mainBowerFiles(), this.conf.bower)
                .pipe(iif(this.conf.useFlatten, flatten))
                .pipe(iif(this.conf.useMinify, minify, this.conf.minify))
                .pipe(gulp.dest(this.conf.path.dest)));
    }
    build() {
        del(this.conf.path.dest)
            .then(() => this.commonPipe(gulp.src(this.conf.path.src)));
    }
    watch() {
        this.commonPipe(watch(this.conf.path.src));
    }
}
